...
async function getData(){
    ...
    let Featured= (await (await fetch(`${process.env.BASE_URL}/api/news/type?type=Featured`)).json())['data']
    let Popular= (await (await fetch(`${process.env.BASE_URL}/api/news/type?type=Popular`)).json())['data']
    let Latest= (await (await fetch(`${process.env.BASE_URL}/api/news/latest`)).json())['data']
